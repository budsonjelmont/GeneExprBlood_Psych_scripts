...
OR <- function(q,k,m,t,alt) {
  ## 2 x 2 table:
  ##         inTest   !inTest
  ## inRef     q        k
  ## !inRef    m        t
  
  fisher.out <- fisher.test(matrix(c(q, k-q, m-q, t-m-k+q), 2, 2),conf.int=TRUE,alternative=alt)
...
ORA <- function(testpath,refpath,testbackground,refbackground,alternative) {
  ...
  empvals <- OR(q,k,m,t,alternative)
